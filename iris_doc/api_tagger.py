...
class SyntaxTokenizer(ABC):

    def findTag(self, fileLines: List[str], startIndex: int, endIndex: int) -> Token:
        """
    ...
    # @abstractmethod
    # def findClassScopeStart(self, line: str) -> bool:
    #     """
...
class DefaultLineScannerV2(LineScanner):
    __syntaxTokenizer: SyntaxTokenizer

    __fileLines: List[str] = []

    def __init__(self, syntaxTokenizer: SyntaxTokenizer, fileLines: List[str]) -> None:
        self.__syntaxTokenizer = syntaxTokenizer
        self.__fileLines = fileLines

    def _findClassScopeStartIndex(self, lines: List[str], startIndex: int) -> int:
        i = startIndex
        while (i < len(lines)):
            if self.SyntaxTokenizer(lines[i]):
                return i
