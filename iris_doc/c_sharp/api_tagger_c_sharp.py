...
class CSharpSyntaxMatcher(LanguageSyntaxMatcher):
    ...
    def matchMemberFunction(self, line: str) -> str:
        ...
        m = re.match(
            r'^(abstract |virtual |override )?(public |protected |private )?([A-Za-z0-9_]+ )([A-Za-z0-9z]+)\(.*\)', line.strip())
        if m:
            return m.group(4)
    ...
    def matchMemberVariable(self, line: str) -> str:
        m = re.match(r'^(public |protected |private )?([A-Za-z0-9_<>]+ )([A-Za-z0-9_]+)(;| \{| \=)', line.strip())
        if m:
            return m.group(3)
